...
|股债对|回测区间|最优参数|final net value|Avg return per round|win rate|total rounds|
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
...
通过横坐标为信号宽度，纵坐标为胜率的二维散点图（详见附录A）可以观察得出：在$[0,0.3]$的区间内，胜率先随着信号宽度的增加会有明显的下降趋势，在信号宽度位于$[0.01,0.02]$的区间内，各组股债对在不同时间段的胜率，都会达到其对应的最小值。在$[0.02,0.03]$区间内，随着信号宽度的增加，胜率也随之上升。
